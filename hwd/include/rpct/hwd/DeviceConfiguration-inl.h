...
DeviceConfiguration::getParameterSetting(Parameter const & _parameter, t_cpp_type & _value) const
{
    ...
    storage_type<t_cpp_type>::set(getConfiguration().getParameterSetting<typename storage_type<t_cpp_type>::sqlite_type>(_setting->second)
                                  , _value);
}
...
{
    if (_parameter.getDataType() != storage_type<t_cpp_type>::datatype())
        throw std::logic_error(std::string("Registering ParameterSetting for wrong DataType (ParameterType ")
                               + _parameter.getParameterType().getName()
                               + ", DataType " + storage_type<t_cpp_type>::datatype().getName() + " instead of " + _parameter.getDataType().getName() + ")");
    if (_parameter.getDeviceType() != getDeviceType())
        throw std::logic_error(std::string("Registering ParameterSetting for different DeviceType (ParameterType ")
                               + _parameter.getParameterType().getName()
                               + ", DeviceType " + _parameter.getDeviceType().getName() + " instead of " + getDeviceType().getName() + ")");
    typename storage_type<t_cpp_type>::sqlite_type _sqlite_value
        = storage_type<t_cpp_type>::get(_value);
