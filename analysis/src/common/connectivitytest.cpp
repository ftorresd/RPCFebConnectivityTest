...
public:
    ...
    bool operator()(rpct::hwd::integer_type _lh, rpct::hwd::integer_type _rh) const
    {
        ...
        return (_lhc.getRoll() < _rhc.getRoll()
                || (_lhc.getRoll() == _rhc.getRoll()
                    && _lhc.getRollConnector() < _rhc.getRollConnector()
                    )
                );
    }
protected:
...
public:
    ...
    bool operator()(rpct::hwd::integer_type _lh, rpct::hwd::integer_type _rh) const
    {
        ...
        return (_lhc.getFebPart().getFebConnector().getRoll() < _rhc.getFebPart().getFebConnector().getRoll()
                || (_lhc.getFebPart().getFebConnector().getRoll() == _rhc.getFebPart().getFebConnector().getRoll()
                    && (_lhc.getFebPart().getFebConnector().getRollConnector() < _rhc.getFebPart().getFebConnector().getRollConnector()
                        || (_lhc.getFebPart().getFebConnector().getRollConnector() == _rhc.getFebPart().getFebConnector().getRollConnector()
                            && _lhc.getPosition() < _rhc.getPosition()
                            )
                        )
                    )
                );
...
void manual(std::string const & _name)
{
    std::cerr << "\n"
              ...
              << "       " << std::setw(_name.size()) << " " << "   [-a rate] [-n rate] [-f fraction]\n"
              << "       " << std::setw(_name.size()) << " " << "   [-s chamber] [-m chamber]\n"
              << "       " << std::setw(_name.size()) << " " << "   [-O output_filename]\n"
              << "       " << std::setw(_name.size()) << " " << "   db_filename\n"
...
int main(int _argc, char * _argv[])
{
    ...
    { // parse parameters
        ...
        while ((_opt = ::getopt(_argc, _argv, "p:i:a:n:f:s:m:O:h")) != -1) {
            ...
            switch (_opt) {
            case 'p':
                _parser << ::optarg; _parser >> _offset;
            ...
            case 'i':
                ...
                break;
            case 'a':
                _parser << ::optarg; _parser >> rate_active_;
                break;
            case 'n':
                _parser << ::optarg; _parser >> rate_noisy_;
                break;
            case 'f':
                _parser << ::optarg; _parser >> channel_fraction_;
                break;
            case 's':
                try {
    ...
    { // Print Configuration
        ...
        *output_ << std::setw(25) << "Selected Rolls: " << std::endl;
        for (std::set<rpct::tools::RollId>::const_iterator _roll
                 = febconntest_.getRollSelection().getSelected().begin()
                 ; _roll != febconntest_.getRollSelection().getSelected().end() ; ++_roll)
            *output_ << std::setw(25) << " " << *_roll << std::endl;
        *output_ << std::setw(25) << "Masked Rolls: " << std::endl;
        for (std::set<rpct::tools::RollId>::const_iterator _roll
                 = febconntest_.getRollSelection().getMasked().begin()
                 ; _roll != febconntest_.getRollSelection().getMasked().end() ; ++_roll)
            *output_ << std::setw(25) << " " << *_roll << std::endl;
        ...
        *output_ << std::setw(25) << "Selected Rolls: " << std::endl;
        for (std::set<rpct::tools::RollId>::const_iterator _roll
                 = roll_selection_.getSelected().begin()
                 ; _roll != roll_selection_.getSelected().end() ; ++_roll)
            *output_ << std::setw(25) << " " << *_roll << std::endl;
        *output_ << std::setw(25) << "Masked Rolls: " << std::endl;
        for (std::set<rpct::tools::RollId>::const_iterator _roll
                 = roll_selection_.getMasked().begin()
                 ; _roll != roll_selection_.getMasked().end() ; ++_roll)
            *output_ << std::setw(25) << " " << *_roll << std::endl;
        *output_ << "--------------------------------------------------------------------------------\n";
    ...
    { // Prepare Histogram Stacks
        // Style Histograms
        TH1F * _linkboard_style_histogram
            = new TH1F("LinkBoardHistogramStyle", "LinkBoardHistogramStyle", 120, .5, 20.5);
        ...
        TH1F * _channel_style_histogram
            = new TH1F("ChannelHistogramStyle", "ChannelHistogramStyle", 96, -.5, 95.5);
        ...
        TH1F * _strip_style_histogram
            = new TH1F("StripHistogramStyle", "StripHistogramStyle", 96, .5, 96.5);
        ...
        for (std::vector<rpct::fct::LinkBox *>::const_iterator _linkbox_it = _linkboxes.begin()
                 ; _linkbox_it != _linkboxes.end() ; ++_linkbox_it) {
            rpct::fct::LinkBox & _linkbox = *(*_linkbox_it);
            bool _linkbox_selected(false);
            for (std::vector<rpct::fct::LinkBoard *>::const_iterator _linkboard_it = _linkbox.getLinkBoards().begin()
                     ; _linkboard_it != _linkbox.getLinkBoards().end() ; ++_linkboard_it)
                if (*_linkboard_it) {
                    rpct::fct::LinkBoard & _linkboard = *(*_linkboard_it);
                    // loop over febconnectors
                    bool _linkboard_selected(false);
                    for (std::vector<rpct::fct::FebConnector *>::const_iterator _febconnector_it = _linkboard.getFebConnectors().begin()
                             ; _febconnector_it != _linkboard.getFebConnectors().end() ; ++_febconnector_it)
                        if (*_febconnector_it && (*_febconnector_it)->isSelected())
                            if (roll_selection_.matches((*_febconnector_it)->getRoll())) {
                                ...
                                if (strip_stacks_.find(_febconnector.getRoll()) == strip_stacks_.end()) {
                                    ...
                                    THStack * _strip_stack
                                        = (strip_stacks_[_febconnector.getRoll()]
                                           = new THStack(name_.str().c_str(), _febconnector.getRoll().name().c_str()));
                                    _strip_stack->SetHistogram(_strip_style_histogram);
                                    _strip_stack->SetMinimum(.9);
                                    _strip_stack->SetMaximum(5e7);
                                    TLegend * _strip_legend
                                        = (strip_legends_[_febconnector.getRoll()]
                                           = new TLegend(.50, .15, .85, .35));
                    ...
                    if (_linkboard_selected) {
                        ...
                        THStack * _channel_stack
                            = (channel_stacks_[_linkboard.getId()]
                               = new THStack(name_.str().c_str(), title_.str().c_str()));
                        _channel_stack->SetHistogram(_channel_style_histogram);
                        _channel_stack->SetMinimum(.9);
                        _channel_stack->SetMaximum(5e7);
                        TLegend * _channel_legend
                            = (channel_legends_[_linkboard.getId()]
                               = new TLegend(.50, .15, .85, .35));
            ...
            if (_linkbox_selected) {
                ...
                THStack * _linkboard_stack
                    = (linkboard_stacks_[_linkbox.getId()]
                       = new THStack(_linkbox.getName().c_str(), title_.str().c_str()));
                _linkboard_stack->SetHistogram(_linkboard_style_histogram);
                _linkboard_stack->SetMinimum(.9);
                _linkboard_stack->SetMaximum(5e7);
                TLegend * _linkboard_legend
                    = (linkboard_legends_[_linkbox.getId()]
                       = new TLegend(.35, .15, .85, .35));
    ...
    { // First snapshot, already assumed
        ...
        for (std::vector<rpct::fct::LinkBox *>::const_iterator _linkbox_it = _linkboxes.begin()
            ...
            if (*_linkbox_it && (*_linkbox_it)->isActive())
                for (std::vector<rpct::fct::LinkBoard *>::const_iterator _linkboard_it = (*_linkbox_it)->getLinkBoards().begin()
                         ; _linkboard_it != (*_linkbox_it)->getLinkBoards().end() ; ++_linkboard_it)
                    if (*_linkboard_it) {
                        rpct::fct::LinkBoard & _linkboard = *(*_linkboard_it);
                        for (std::vector<rpct::fct::FebConnector *>::const_iterator _febconnector_it = _linkboard.getFebConnectors().begin()
                                 ; _febconnector_it != _linkboard.getFebConnectors().end() ; ++_febconnector_it)
                            if (*_febconnector_it && roll_selection_.matches((*_febconnector_it)->getRoll())) {
                                ...
                                { // offset?
                                    ...
                                    if (std::abs((int)_febchip_0.getVThRead() - (int)_febchip_0.getVThSet()) > vth_offset_ignore_) {
                                        ...
                                        offset_chip_ignore_.insert(std::pair<rpct::hwd::integer_type, int>(_febchip_0.getId()
                                                                                                           ,(int) _febchip_0.getVThRead() - _febchip_0.getVThSet()));
                                    }
                                    if (std::abs((int)_febchip_1.getVThRead() - (int)_febchip_1.getVThSet()) > vth_offset_ignore_) {
                                        _ignore_connector = true;
                                        offset_chip_ignore_.insert(std::pair<rpct::hwd::integer_type, int>(_febchip_1.getId()
                                                                                                           , (int)_febchip_1.getVThRead() - _febchip_1.getVThSet()));
                                    }
                                    if (_ignore_connector)
                                        offset_connector_ignore_index_.insert(_febconnector.getId());
                                }

                                if (!_ignore_connector) { // noisy?
                                    ...
                                    for (int _pin = 1 ; _pin <= rpct::fct::FebConnectorStrips::npins_ ; ++_pin, ++_channel)
                                        if (_febconnector.getStrips().isPinActive(_pin)
                                            && _linkboard.getRate(_channel) > rate_noisy_)
                                            ++_noisy_channels;
    ...
    int _nsnapshots(_snapshots.size())
        , _nsnapshot(1);
    // Loop over Snapshots
    for (std::vector<rpct::hwd::Snapshot>::const_iterator _snapshot = _snapshots.begin()
             ; _snapshot != _snapshots.end() ; ++_snapshot, ++_nsnapshot) {
        ...
        if (febconntest_.getActiveFebConnectorId() // skip the first connectivitytest step
            ...
            { // offset?
                ...
                if (offset_chip_ignore_.find(_febchip_0.getId()) == offset_chip_ignore_.end()
                    && std::abs((int)_febchip_0.getVThRead() - (int)_febchip_0.getVThSet()) > vth_offset_problem_)
                    offset_chip_problem_.insert(std::pair<rpct::hwd::integer_type, int>(_febchip_0.getId()
                                                                                        ,(int) _febchip_0.getVThRead() - _febchip_0.getVThSet()));
                if (offset_chip_ignore_.find(_febchip_1.getId()) == offset_chip_ignore_.end()
                    && std::abs((int)_febchip_1.getVThRead() - (int)_febchip_1.getVThSet()) > vth_offset_problem_)
                    offset_chip_problem_.insert(std::pair<rpct::hwd::integer_type, int>(_febchip_1.getId()
                                                                                        , (int)_febchip_1.getVThRead() - _febchip_1.getVThSet()));
            }
            ...
            { // active?
                ...
                for (int _pin = 1 ; _pin <= rpct::fct::FebConnectorStrips::npins_ ; ++_pin, ++_channel)
                    if (_active_febconnector.getStrips().isPinActive(_pin)
                        && _active_linkboard.getRate(_channel) > rate_active_)
                        ++(_active_channels[(_pin - 1) / (rpct::fct::FebConnectorStrips::npins_ / 2)]);

                if ((double)(_active_channels[0] + _active_channels[1]) / (double)_active_febconnector.getStrips().getNStrips() < channel_fraction_
                    && offset_connector_ignore_index_.find(_active_febconnector.getId()) == offset_connector_ignore_index_.end()) {
                    dead_connector_.push_back(_active_febconnector.getId());
                    _active_is_dead = true;
                }
                else if (((double)(_active_channels[0]) / (double)_active_febconnector.getStrips().getNStrips() < channel_fraction_
                          || (double)(_active_channels[1]) / (double)_active_febconnector.getStrips().getNStrips() < channel_fraction_)
                         && offset_connector_ignore_index_.find(_active_febconnector.getId()) == offset_connector_ignore_index_.end())
            ...
            if (linkboard_histograms_.find(_active_linkboard.getId()) == linkboard_histograms_.end()) {
                ...
                TH1F * _linkboard_indicator_histogram
                    = new TH1F(name_.str().c_str(), title_.str().c_str(), 120, .5, 20.5);
                _linkboard_indicator_histogram->SetFillColor(_linkboard_color);
                _linkboard_indicator_histogram->SetLineColor(_linkboard_color);
                for (std::vector<rpct::fct::FebConnector *>::const_iterator _febconnector_it = _active_linkboard.getFebConnectors().begin()
                         ; _febconnector_it != _active_linkboard.getFebConnectors().end() ; ++_febconnector_it)
                    if (*_febconnector_it && (*_febconnector_it)->isSelected())
                ...
                TH1F * _linkboard_histogram =
                    ( linkboard_histograms_[_active_linkboard.getId()]
                      = new TH1F(name_.str().c_str(), title_.str().c_str(), 120, .5, 20.5));
            ...
            TH1F * _channel_indicator_histogram
                = new TH1F(name_.str().c_str(), title_.str().c_str(), 96, -.5, 95.5);
            ...
            TH1F * _strip_indicator_histogram
                = new TH1F(name_.str().c_str(), title_.str().c_str(), 96, .5, 96.5);
            ...
            for (std::vector<rpct::fct::LinkBoard *>::const_iterator _linkboard_it = _active_linkbox.getLinkBoards().begin()
                     ; _linkboard_it != _active_linkbox.getLinkBoards().end() ; ++_linkboard_it)
                if (*_linkboard_it)
                    for (std::vector<rpct::fct::FebConnector *>::const_iterator _febconnector_it = (*_linkboard_it)->getFebConnectors().begin()
                             ; _febconnector_it != (*_linkboard_it)->getFebConnectors().end() ; ++_febconnector_it)
                        if (*_febconnector_it) {
                            rpct::fct::FebConnector const & _febconnector(*(*_febconnector_it));
                            if (_febconnector.isSelected())
                                _linkboard_histogram->AddBinContent(((*_linkboard_it)->getPosition() - 1) * rpct::fct::LinkBoard::nfebconnectors_ + (*_febconnector_it)->getPosition()
                                                                    , (*_linkboard_it)->getAvgRate((*_febconnector_it)->getPosition()));
                            if (_febconnector.getRoll().getChamberId() == _active_chamber) {
                                if (strip_histograms_.find(_febconnector.getRoll()) == strip_histograms_.end()) {
                                    name_.str("");
                                    name_ << "RPCDetId_" << _febconnector.getRoll().getRPCDetId().id();
                                    TH1F * _strip_histogram
                                        = (strip_histograms_[_febconnector.getRoll()]
                                           = new TH1F(name_.str().c_str()
                                                      , _active_roll.name().c_str(), 96, .5, 96.5));
                                ...
                                for (int _pin = 1 ; _pin <= rpct::fct::FebConnectorStrips::npins_ ; ++_pin, ++_channel)
                                    if (_febconnector.getStrips().isPinActive(_pin))
                                        _strip_histogram->SetBinContent(_febconnector.getStrips().getStrip(_pin)
                                                                        , _febconnector.getLinkBoard().getRate(_channel));

                            }
                            if (_febconnector != _active_febconnector) { // active or noisy?
                                int _channel = (_febconnector.getPosition() - 1) * rpct::fct::FebConnectorStrips::npins_;
                                ...
                                if (_active_is_dead
                                    && (double)_active_channels / (double)_febconnector.getStrips().getNStrips() > channel_fraction_
                                    && offset_connector_ignore_index_.find(_febconnector.getId()) == offset_connector_ignore_index_.end()
                                    && noisy_connector_ignore_index_.find(_febconnector.getId()) == noisy_connector_ignore_index_.end())
            ...
            for (std::map<rpct::tools::RollId, TH1F *>::iterator _strip_histogram_it = strip_histograms_.begin()
                     ; _strip_histogram_it != strip_histograms_.end() ; ++_strip_histogram_it)
                _strip_histogram_it->second->Write();
            strip_histograms_.clear();

            // Channel Histogram
            TH1F * _channel_histogram
                = new TH1F(_active_linkboard.getName().c_str()
                           , _active_linkboard.getName().c_str(), 96, -.5, 95.5);
    ...
    for (std::map<rpct::hwd::integer_type, TH1F *>::iterator _linkboard_histogram_it = linkboard_histograms_.begin()
             ; _linkboard_histogram_it != linkboard_histograms_.end() ; ++_linkboard_histogram_it)
    ...
    for (std::map<rpct::hwd::integer_type, THStack *>::iterator _linkboard_stack = linkboard_stacks_.begin()
             ; _linkboard_stack != linkboard_stacks_.end() ; ++_linkboard_stack) {
        working_directory_->cd();
        _linkboard_stack->second->Write();
        linkboard_legends_[_linkboard_stack->first]->Write();
        root_file_.cd();
        TCanvas * _canvas
            = new TCanvas(_linkboard_stack->second->GetName());
        _canvas->SetLogy();
        _canvas->SetGridy();
    ...
    for (std::map<rpct::hwd::integer_type, THStack *>::iterator _channel_stack = channel_stacks_.begin()
             ; _channel_stack != channel_stacks_.end() ; ++_channel_stack) {
        working_directory_->cd();
        _channel_stack->second->Write();
        channel_legends_[_channel_stack->first]->Write();
        root_file_.cd();
        TCanvas * _canvas
            = new TCanvas(_channel_stack->second->GetName());
        _canvas->SetLogy();
        _canvas->SetGridy();
    ...
    for (std::map<rpct::tools::RollId, THStack *>::iterator _strip_stack = strip_stacks_.begin()
             ; _strip_stack != strip_stacks_.end() ; ++_strip_stack) {
        working_directory_->cd();
        _strip_stack->second->Write();
        strip_legends_[_strip_stack->first]->Write();
        root_file_.cd();
        TCanvas * _canvas
            = new TCanvas(_strip_stack->second->GetName());
        _canvas->SetLogy();
        _canvas->SetGridy();
    ...
    if (!(offset_chip_ignore_.empty())) {
        ...
        *output_ << std::setw(25) << "Roll"
        ...
        for (std::map<rpct::hwd::integer_type, int>::const_iterator _chip = offset_chip_ignore_.begin()
                 ; _chip != offset_chip_ignore_.end() ; ++_chip) {
    ...
    if (!(noisy_connector_ignore_.empty())) {
        ...
        *output_ << std::setw(25) << "Roll_Connector"
        ...
        for (std::vector<rpct::hwd::integer_type>::const_iterator _connector = noisy_connector_ignore_.begin()
                 ; _connector != noisy_connector_ignore_.end() ; ++_connector) {
    ...
    if (!(offset_chip_problem_.empty())) {
        ...
        *output_ << std::setw(25) << "Roll"
                 ...
                 << std::setw(11) << "ID"
                 << std::setw(11) << "Offset" << std::endl;
        for (std::map<rpct::hwd::integer_type, int, FebChipCompare>::const_iterator _chip = offset_chip_problem_.begin()
                 ; _chip != offset_chip_problem_.end() ; ++_chip) {
            rpct::fct::FebChip const & _febchip = (rpct::fct::FebChip const &)(febsystem_.getDevice(_chip->first));
            *output_ << std::setw(25) << _febchip.getFebPart().getFebConnector().getRoll()
                     << " " << std::setw(10) << _febchip.getPosition()
                     << " " << std::setw(10) << _febchip.getFebPart().getFebBoard().getFebDistributionBoard().getControlBoard().getLinkBox().getLocation()
    ...
    if (!(noisy_connector_problem_.empty())) {
        ...
        *output_ << std::setw(25) << "Roll_Connector"
                 ...
                 << std::setw(4) << "LB"
                 << std::setw(4) << "Pos" << std::endl;
        for (std::vector<rpct::hwd::integer_type>::const_iterator _connector = noisy_connector_problem_.begin()
                 ; _connector != noisy_connector_problem_.end() ; ++_connector) {
            rpct::fct::FebConnector const & _febconnector = (rpct::fct::FebConnector const &)(febsystem_.getDevice(*_connector));
            *output_ << std::setw(22) << _febconnector.getRoll() << "_" << std::setw(2) << std::setfill('0') << _febconnector.getRollConnector() << std::setfill(' ')
                     << " " << std::setw(10) << _febconnector.getLinkBoard().getLinkBox().getLocation()
                     << " " << std::setw(3) << _febconnector.getLinkBoard().getPosition()
    ...
    if (!(dead_connector_.empty()) && dead_connector_.size() != swap_connectors_.size()) {
        ...
        *output_ << std::setw(25) << "Roll_Connector"
                 ...
                 << std::setw(4) << "LB"
                 << std::setw(4) << "Pos" << std::endl;
        for (std::vector<rpct::hwd::integer_type>::const_iterator _connector = dead_connector_.begin()
                 ; _connector != dead_connector_.end() ; ++_connector)
    ...
    if (!(partially_dead_connector_.empty())) {
        ...
        *output_ << std::setw(25) << "Roll_Connector"
                 ...
                 << std::setw(4) << "LB"
                 << std::setw(4) << "Pos" << std::endl;
        for (std::vector<rpct::hwd::integer_type>::const_iterator _connector = partially_dead_connector_.begin()
                 ; _connector != partially_dead_connector_.end() ; ++_connector)
            if (swap_connectors_.find(*_connector) == swap_connectors_.end()) {
                rpct::fct::FebConnector const & _febconnector = (rpct::fct::FebConnector const &)(febsystem_.getDevice(*_connector));
                *output_ << std::setw(22) << _febconnector.getRoll() << "_" << std::setw(2) << std::setfill('0') << _febconnector.getRollConnector() << std::setfill(' ')
                         << " " << std::setw(10) << _febconnector.getLinkBoard().getLinkBox().getLocation()
    ...
    if (!(swap_connectors_.empty())) {
        ...
        *output_ << std::setw(30) << "Roll_Connector"
        ...
        for (std::map<rpct::hwd::integer_type, std::vector<rpct::hwd::integer_type> >::iterator swap_connectors_it = swap_connectors_.begin()
                 ; swap_connectors_it != swap_connectors_.end() ; ++swap_connectors_it) {
            ...
            *output_ << std::setw(27) << _source.getRoll() << "_" << std::setw(2) << std::setfill('0') << _source.getRollConnector() << std::setfill(' ')
            ...
            for (std::vector<rpct::hwd::integer_type>::const_iterator _candidates_it = swap_connectors_it->second.begin()
                     ; _candidates_it != swap_connectors_it->second.end() ; ++_candidates_it) {
